...
# Requirements: Windows 10+, Claude Desktop, Node.js (install from nodejs.org if missing)
...
$PlaywrightCli = Join-Path $RepoDir "node_modules\.bin\playwright.cmd"
if (Test-Path $PlaywrightCli) {
    & $PlaywrightCli install chromium
    if ($LASTEXITCODE -eq 0) {
        Write-Host "   Playwright Chromium installed"
    } else {
        Write-Host "   WARNING: Playwright install had warnings - Alfred will attempt to use system Chromium"
    }
} else {
    Write-Host "   WARNING: Playwright CLI not found - run 'npm ci' first"
}

Write-Host ""
Write-Host "[>] Removing old Alfred Chrome launcher (no longer needed)..."
$DesktopPath = [Environment]::GetFolderPath("Desktop")
if (-not $DesktopPath -or -not (Test-Path $DesktopPath)) {
    $DesktopPath = Join-Path $env:USERPROFILE "Desktop"
}
$OldBat = Join-Path $DesktopPath "Alfred.bat"
$RemovedLauncher = $false
if (Test-Path $OldBat) {
    Remove-Item $OldBat -Force
    Write-Host "   Deleted Alfred.bat from Desktop"
    $RemovedLauncher = $true
}
if (-not $RemovedLauncher) {
    Write-Host "   No old launcher found - nothing to remove"
}
Write-Host "   Alfred now launches its own browser automatically - no launcher needed"
...
Write-Host "  2. Ask Claude anything - Alfred will open a browser window automatically"
Write-Host "  3. Log into Dynamics, Outlook and Teams the first time (never again after that)"
Write-Host "  4. Done - ask Claude about opportunities, calendar, hygiene sweep!"
Write-Host ""
