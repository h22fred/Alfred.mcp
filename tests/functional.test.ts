...
describe("sanitizeODataSearch", () => {
  it("passes through safe characters", () => {
    expect(sanitizeODataSearch("Acme Corp")).toBe("Acme Corp");
    expect(sanitizeODataSearch("Fabrikam-2026")).toBe("Fabrikam-2026");
  });
...
describe("formatHygieneReport", () => {
  ...
  it("groups by account", () => {
    const results = [
      makeResult({ opportunity: { ...makeResult().opportunity, accountName: "Acme Corp" } }),
      makeResult({ opportunity: { ...makeResult().opportunity, accountName: "Acme Corp" } }),
      makeResult({ opportunity: { ...makeResult().opportunity, accountName: "Fabrikam" } }),
    ];
    const report = formatHygieneReport(results);
    expect(report).toContain("Acme Corp");
    expect(report).toContain("Fabrikam");
  });
