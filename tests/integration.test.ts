...
describe("getCalendarEvents Graph API error handling", () => {
  ...
  beforeEach(async () => {
    ...
    // Pre-seed Outlook REST token — calendar uses same path as email
...
describe("opportunity mapping via fetchOpportunities", () => {
  ...
  it("maps all fields from a complete Dynamics response", async () => {
    const raw = {
      ...
      name: "Acme Corp ITSM Expansion",
      parentaccountid: { accountid: "acc-sita", name: "Acme Corp" },
      statuscode: 1,
      ...
      description: "Major ITSM expansion for Acme Corp",
    ...
    expect(opp.name).toBe("Acme Corp ITSM Expansion");
    expect(opp.accountName).toBe("Acme Corp");
    ...
    expect(opp.description).toBe("Major ITSM expansion for Acme Corp");
...
describe("sanitizeODataSearch in OData URL context", () => {
  ...
  it("produces a valid OData filter URL with sanitized input", () => {
    ...
    expect(url).toContain("contains(name,'Acme Corp')");
...
describe("post-meeting candidate matching", () => {
  ...
  it("subject matching: prefers subject match over domain match", () => {
    ...
    const opportunities = [
      { opportunityid: "opp-1", name: "Acme Corp ITSM", accountName: "Acme Corp" },
  ...
  it("domain matching: falls back to domain when no subject match", () => {
    ...
    const opportunities = [
      { opportunityid: "opp-1", name: "Acme Corp Renewal", accountName: "Acme Corp" },
    ];
